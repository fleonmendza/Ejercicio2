...
const getInfoPokemon = async (url) => {
  ...
  cards.innerHTML += `
       <div id="card">
        ...
        <button id="btnAgregarCarrito" onclick="addPokemonList('${name}',${id} , '${img}' );" class="btnAgregarCarrito" >Agregar al Carrito
...
const addPokemonList = (name,id,img) =>{
  console.log(`${name}`,id,img);

  Carrito.innerHTML += `
              <div id="CarritoItem" class="hidden">
                <img id="imgCarrito" src="${img}" alt="">
                <span>${name}</span>
                <span>${id}</span>
...
document.addEventListener('click', (e)=>{ 
  if(e.target===btnCarrito){
    
    carrito.classList.toggle('hidden');
    carritoItem.classList.toggle('hidden');
  }

  
  // carrito.classList.toggle('hidden');
  // compras.classList.toggle('hidden')
});   
